...
import com.playjava.mapper.TOrderMapper;
import com.playjava.mapper.TOrderItemMapper;
import com.playjava.mapper.MCustomerMapper;
...
import com.playjava.service.adapter.OrderService;
...
public class OrderServiceImpl extends ServiceImpl<TOrderMapper, TOrder> implements OrderService {
    ...
    public String createStorePurchaseOrderImpl(CreateOrderRequest request) {
        // 1. 顧客の存在確認
        MCustomer customer = mCustomerMapper.selectById(request.getCustomerId());
        Option.of(customer)
            .filter(c -> !Boolean.TRUE.equals(c.getDeleteFlag()))
            .getOrElseThrow(() -> new RuntimeException("顧客が存在しません: customerId=" + request.getCustomerId()));
        ...
        for (OrderItemRequest itemRequest : request.getItems()) {
            ...
            // 在庫確認（productIdで検索）
            LambdaQueryWrapper<MStock> stockWrapper = new LambdaQueryWrapper<>();
            stockWrapper.eq(MStock::getProductId, itemRequest.getProductId());
            MStock stock = mStockMapper.selectOne(stockWrapper);
            Option.of(stock)
                .filter(s -> !Boolean.TRUE.equals(s.getDeleteFlag()))
                .getOrElseThrow(() -> new RuntimeException("在庫レコードが存在しません: productId=" + itemRequest.getProductId()));
    ...
    public boolean confirmOrderImpl(String orderId) {
        ...
        for (TOrderItem orderItem : orderItems) {
            // 4-1. 在庫確認
            LambdaQueryWrapper<MStock> stockWrapper = new LambdaQueryWrapper<>();
            stockWrapper.eq(MStock::getProductId, orderItem.getProductId());
            MStock stock = mStockMapper.selectOne(stockWrapper);
            Option.of(stock)
                .filter(s -> !Boolean.TRUE.equals(s.getDeleteFlag()))
                .getOrElseThrow(() -> new RuntimeException("在庫レコードが存在しません: productId=" + orderItem.getProductId()));
            
            // 4-2. 在庫数量チェック
            if (stock.getQuantity() < orderItem.getQuantity()) {
                throw new RuntimeException("在庫不足です: productId=" + orderItem.getProductId() + 
                    ", 在庫数量=" + stock.getQuantity() + ", 注文数量=" + orderItem.getQuantity());
            }
    ...
    /**
     * 注文検索処理（ページング・ソート対応）
     * @param orderId 注文ID（完全一致）
     * @param customerId 顧客ID（完全一致）
     * @param orderDateFrom 注文日時開始（範囲指定）
     * @param orderDateTo 注文日時終了（範囲指定）
     * @param status ステータス（完全一致）
     * @param deleteFlag 削除フラグ（true/false/null、未指定時はfalseのみ）
     * @param pageNum ページ番号（1から開始）
     * @param pageSize ページサイズ
     * @param sortBy ソート対象カラム（orderId, customerId, orderDate, status, createDate, updateDate）
    ...
    public boolean cancelOrderImpl(String orderId) {
        ...
        if (currentStatus == 1) {
            ...
            for (TOrderItem orderItem : orderItems) {
                ...
                // 在庫を戻す
                stock.setQuantity(stock.getQuantity() + orderItem.getQuantity());
                // 在庫ステータスの更新（在庫が0より大きくなったら0: 在庫あり）
                if (stock.getQuantity() > 0 && stock.getStatus() == 1) {
                    stock.setStatus(0);
                }
                mStockMapper.updateById(stock);
            }
